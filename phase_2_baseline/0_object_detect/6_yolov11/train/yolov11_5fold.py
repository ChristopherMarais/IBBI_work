...
BASE_RESULTS_DIR = "./results_yolo11x" # Main directory for all cross-validation results
MODEL_NAME_OR_PATH = "yolo11x.pt" # MODIFIED: For yolo11x

# --- Training Hyperparameters ---
TRAIN_HYPERPARAMS = {
    "epochs": 3,
    "batch": 32,             # Batch size per GPU. Adjust based on your GPU memory for yolo11x
...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler(os.path.join(main_orchestration_log_dir, "cv_orchestration_main_yolo11x.log")),
...
def train_single_fold(fold_idx, assigned_gpu_id):
    ...
    try:
        with open(stdout_stderr_log_file, 'w') as f_log:
            ...
            # IMPORTANT: Ensure 'yolo11x.pt' exists or is a valid model specifier for Ultralytics.
            # If it's a custom model, ensure the path is correct.
...
if __name__ == '__main__':
    # Note: yolo11x is a hypothetical model name for this script.
    # Ensure that 'yolo11x.pt' (or your intended model file/name) is accessible
    # and that your Ultralytics environment supports it.
    # Adjust batch size and other hyperparameters based on yolo11x's requirements and your hardware.
    logging.info(f"Starting yolo11x training script. Make sure '{MODEL_NAME_OR_PATH}' is a valid model.")
